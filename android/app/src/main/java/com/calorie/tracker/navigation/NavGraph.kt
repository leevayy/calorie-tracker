...
import com.calorie.tracker.ui.reference.ReferenceFoodsScreen
import com.calorie.tracker.ui.reference.ReferenceFoodsViewModel
...
fun AppNavGraph(
    ...
    referenceFoodDb: ReferenceFoodDb? = null,
    startRoute: String,
    isDarkMode: Boolean,
    ...
    key(key) {
        ...
        val mainRoutes = listOf(Routes.HOME, Routes.HISTORY, Routes.REFERENCE, Routes.SETTINGS)
        val showBottomBar = currentRoute in mainRoutes

        val titles = mapOf(
            Routes.HOME to R.string.app_name,
            Routes.HISTORY to R.string.history_title,
            Routes.REFERENCE to R.string.reference_title,
        ...
        Scaffold(
                ...
                if (showBottomBar) {
                    NavigationBar {
                        ...
                        NavigationBarItem(
                            ...
                            selected = navEntry?.destination?.hierarchy?.any { it.route == Routes.REFERENCE } == true,
                            onClick = {
                                navController.navigate(Routes.REFERENCE) {
                                    popUpTo(Routes.HOME)
                                    launchSingleTop = true
                                }
                            }
            ...
            NavHost(
                ...
                composable(Routes.HOME) {
                    ...
                    MainScreen(viewModel = vm, referenceFoodDb = referenceFoodDb)
                }
                ...
                composable(Routes.REFERENCE) {
                    val vm = viewModel<ReferenceFoodsViewModel> {
                        ReferenceFoodsViewModel(referenceFoodDb!!)
                    }
                    ReferenceFoodsScreen(viewModel = vm)
                }
